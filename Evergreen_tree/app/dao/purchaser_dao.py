...
import pymysql
from app.dao.sql.sql_purchaser import sql_purchaser
import json
...
def set_purchaser(usertelephone):
    try:
        client=POOL.connection()#链接池创建链接
        res_data = None#定义返回值
        cursor = client.cursor(cursor=pymysql.cursors.DictCursor)#游标 操作数据和获取数据库结果都要通过游标来操作
        # 插入数据的sql语句格式
        sql = sql_purchaser.get('set_purchaser').format(telephone=usertelephone['telephone'])#sql语句
...
def get_purchaser(user_id):
    try:
        client=POOL.connection()#链接池创建链接
        res_data = None#定义返回值
        cursor = client.cursor(cursor=pymysql.cursors.DictCursor)#游标 操作数据和获取数据库结果都要通过游标来操作
        sql = sql_purchaser.get('get_purchaser').format(user_id=user_id['user_id'])#sql语句
        cursor.execute(sql)#执行sql语句
        res_data = cursor.fetchall()#获取返回的数据  只取一条
        client.commit()#链接提交事务  还有游标结束
    except Exception as ex:
